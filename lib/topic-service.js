...
TopicService.prototype.add = function (topicName, data) {
    ...
    if (remote) {
        ...
        remote.onResult(function(err){
            if (err) {
                console.log('Result ERROR: topicService!');
                return restoreMsg.call(self, topicName, msg);
            }

            console.log('Result OK: topicService!');
        });

        remote.send(null, msg);
    }
};

TopicService.prototype.addRemote = function (topicName, remote) {
    var self = this;

    addRmt.call(self, topicName, remote);
...
function addRmt(topicName, remote) {
